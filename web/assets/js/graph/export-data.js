	...
	q(c, "mixins/download-url.js", [c["parts/Globals.js"]], function (b) {
		...
		b.dataURLtoBlob =
			function (b) {
				if ((b = b.match(/data:([^;]*)(;base64)?,([0-9A-Za-z+/]+)/)) && 3 < b.length && a.atob && a.ArrayBuffer && a.Uint8Array && a.Blob && h.createObjectURL) {
					...
					for (var d = 0; d < e.length; ++d) e[d] = c.charCodeAt(d);
					b = new a.Blob([e], {
						type: b[1]
					});
		...
		b.downloadURL = function (e, h) {
			...
			if ("string" === typeof e || e instanceof String || !c.msSaveOrOpenBlob) {
				if (f || 2E6 < e.length)
					if (e = b.dataURLtoBlob(e), !e) throw Error("Failed to convert to blob");
				if (void 0 !== p.download) p.href = e, p.download = h, q.body.appendChild(p), p.click(), q.body.removeChild(p);
				else try {
	...
	q(c, "modules/export-data.src.js", [c["parts/Globals.js"], c["parts/Utilities.js"]], function (b, a) {
		...
		b.Chart.prototype.getDataRows = function (a) {
			...
			this.series.forEach(function (k) {
				...
				if (!1 !== k.options.includeInDataExport && !k.options.isInternal && !1 !== k.visible) {
					...
					for (m = 0; m < r;) y = z(k, t[m], t.length), n.push(y.columnTitle ||
						y), a && p.push(y.topLevelColumnTitle || y), m++;
					var q = {
						chart: k.chart,
					...
					k.options.data.forEach(function (a, b) {
						...
						d[a].x = g.x;
						d[a].name = l;
						for (d[a].xValues[f] = g.x; m < r;) b = t[m], l = g[b], d[a][u + m] = e(B.categoryMap[b][l],
							B.dateTimeValueAxisMap[b] ? h.dateFormat(w.dateFormat, l) : null, l), m++
					});
					u += m
				}
			});
			for (r in d) Object.hasOwnProperty.call(d, r) && f.push(d[r]);
			var r = a ? [p, n] : [n];
			for (u = g.length; u--;) {
				var t = g[u][0];
				var v = g[u][1];
				...
				f.forEach(function (a) {
					var b = a.name;
					x && !q(b) && (x.isDatetimeAxis ? (a.x instanceof Date && (a.x = a.x.getTime()), b = h.dateFormat(w.dateFormat, a.x)) : b = x.categories ? e(x.names[a.x], x.categories[a.x],
						a.x) : a.x);
					a.splice(v, 0, b)
				})
			}
		...
		b.Chart.prototype.getCSV = function (a) {
			...
			c.forEach(function (a, d) {
				for (var e, m = a.length; m--;) e = a[m], "string" === typeof e && (e = '"' + e + '"'), "number" === typeof e && "." !== l && (e = e.toString().replace(".", l)), a[m] = e;
				b += a.join(h);
				d < c.length - 1 && (b +=
					f)
			});
		...
		b.Chart.prototype.getTable = function (a) {
			...
			!1 !== c.exporting.tableCaption && (b +=
				'<caption class="highcharts-table-caption">' + e(c.exporting.tableCaption, c.title.text ? c.title.text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#x27;").replace(/\//g, "&#x2F;") : "Chart") + "</caption>");
			for (var p = 0, z = a.length; p < z; ++p) a[p].length > f && (f = a[p].length);
			b += function (a, b, e) {
				var d = "<thead>",
					f = 0;
				e = e || b && b.length;
				...
				if (b) {
					d += "<tr>";
					f = 0;
					for (e = b.length; f < e; ++f) void 0 !== b[f] && (d += n("th", null, 'scope="col"', b[f]));
					d += "</tr>"
				}
				return d + "</thead>"
			...
			a.forEach(function (a) {
				b += "<tr>";
				for (var c = 0; c < f; c++) b += n(c ? "td" : "th", null, c ? "" : 'scope="row"', a[c]);
				b += "</tr>"
			});
			b += "</tbody></table>";
			a = {
		...
		}), a.buttons && a.buttons.contextButton.menuItems.push("separator", "downloadCSV", "downloadXLS",
			"viewData", "openInCloud");
